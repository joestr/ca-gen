...
openssl req -config ./openssl.conf -new -sha512 -key ./root-ca/root-ca.pem -out ./root-ca/root-ca.csr -subj "/C=US/ST=California/L=Las Quejas/O=YouTube Ministry of Pendatry/OU=Trust Provider/CN=YouTube MoP Trust Provider Root/emailAddress=moptrust@youtube.PLACEHOLDER.com"
...
openssl req -config ./openssl.conf -new -sha512 -key ./root-ca/certs/root-ca_ocsp/root-ca_ocsp.pem -out ./root-ca/certs/root-ca_ocsp/root-ca_ocsp.csr -subj "/C=US/ST=California/L=Las Quejas/O=YouTube Ministry of Pendatry/OU=Trust Provider/CN=YouTube MoP Trust Provider Root - OCSP/emailAddress=moptrust@youtube.PLACEHOLDER.com"
...
openssl genrsa -out ./root-ca/certs/tls-ca/tls-ca.pem 1024
openssl req -config ./openssl.conf -new -sha512 -key ./root-ca/certs/tls-ca/tls-ca.pem -out ./root-ca/certs/tls-ca/tls-ca.csr -subj "/C=US/ST=California/L=Las Quejas/O=YouTube Ministry of Pendatry/OU=Trust Provider/CN=YouTube MoP Trust Provider TLS Signing/emailAddress=moptrust@youtube.PLACEHOLDER.com"
openssl ca -config ./openssl.conf -name tls-ca -batch -notext -in ./root-ca/certs/tls-ca/tls-ca.csr -out ./root-ca/certs/tls-ca/tls-ca.crt

mkdir -p ./tls-ca/{newcerts,certs,crl}
touch ./tls-ca/{tls-ca.serial.txt,tls-ca-index.txt,tls-ca.crlserial.txt}

# Revoke TLS CA #
#openssl ca -name tls-ca -revoke ./root-ca/certs/tls-ca/tls-ca.crt
#openssl ca -name tls-ca -gencrl -out /root-ca/root-ca.crl
...
openssl genrsa -out ./root-ca/certs/email-ca/email-ca.pem 1024
openssl req -config ./openssl.conf -new -sha512 -key ./root-ca/certs/email-ca/email-ca.pem -out ./root-ca/certs/email-ca/email-ca.csr -subj "/C=US/ST=California/L=Las Quejas/O=YouTube Ministry of Pendatry/OU=Trust Provider/CN=YouTube MoP Trust Provider Email Signing/emailAddress=moptrust@youtube.PLACEHOLDER.com"
openssl ca -config ./openssl.conf -name email-ca -batch -notext -in ./root-ca/certs/email-ca/email-ca.csr -out ./root-ca/certs/email-ca/email-ca.crt

mkdir -p ./email-ca/{newcerts,certs,crl}
touch ./email-ca/{email-ca.serial.txt,email-ca-index.txt,email-ca.crlserial.txt}

# Revoke Email CA #
#openssl ca -name email-ca -revoke ./root-ca/certs/email-ca/email-ca.crt
#openssl ca -name email-ca -gencrl -out ./root-ca/root-ca.crl
...
openssl genrsa -out ./root-ca/certs/code-ca/code-ca.pem 1024
openssl req -config ./openssl.conf -new -sha512 -key ./root-ca/certs/code-ca/code-ca.pem -out ./root-ca/certs/code-ca/code-ca.csr -subj "/C=US/ST=California/L=Las Quejas/O=YouTube Ministry of Pendatry/OU=Trust Provider/CN=YouTube MoP Trust Provider Code Signing/emailAddress=moptrust@youtube.PLACEHOLDER.com"
openssl ca -config ./openssl.conf -name code-signing-ca -batch -notext -in ./root-ca/certs/code-ca/code-ca.csr -out ./root-ca/certs/code-ca/code-ca.crt

mkdir -p ./code-ca/{newcerts,certs,crl}
touch ./code-ca/{code-ca.serial.txt,code-ca-index.txt,code-ca.crlserial.txt}

# Revoke Code CA #
#openssl ca -name code-signing-ca -revoke ./root-ca/certs/code-ca/code-ca.crt
#openssl ca -name code-signing-ca -gencrl -out ./root-ca/root-ca.crl
